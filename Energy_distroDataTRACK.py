...
def zonesSelection(dfa,lat,lon):
    ...
    dfb=dfa[np.absolute(dfa[lat])<3]
    print("cut on l longitude")
    dfc=dfb[(dfb[lon]<40) | (dfb[lon]>320)]
    return dfc
...
def checker_off_zone(df):
    ...
    shifted=c.transform_to(frame_common)
    print(shifted)
    for i in range(10):   #for i in range(10):
        sh=4.5/24+i*(1-4.5/24-7.2/24)/9
        shift=Timemjd+sh
        ...
        print("final gal",gal)
        off=[]
        off=np.zeros(len(gal.l))
        for coor in range(len(gal.l)):
            if np.absolute(float(gal[coor].b.deg))<3:
                if float(gal[coor].b.deg)<40 or float(gal[coor].b.deg)>320:
                    off[coor]=1
                else:
                    continue
            else:
                continue
        df['off'+str(i)]=off
        df['gal_b'+str(i)]=gal.b.deg
        df['gal_l'+str(i)]=gal.l.deg
...
def plot_OFFZONE(df):
    ...
    #---------------------------------------
    ll=list(df0['gal_l0'])+list(df1['gal_l1'])+list(df2['gal_l2'])+list(df3['gal_l3'])+list(df4['gal_l4'])+list(df5['gal_l5'])+list(df6['gal_l6'])+list(df7['gal_l7'])+list(df8['gal_l8'])
    bb=list(df0['gal_b0'])+list(df1['gal_b1'])+list(df2['gal_b2'])+list(df3['gal_b3'])+list(df4['gal_b4'])+list(df5['gal_b5'])+list(df6['gal_b6'])+list(df7['gal_b7'])+list(df8['gal_b8'])
    #plotter(ll,bb,'galactic')    
    #---------------------------------------
...
def plotter(array_l,array_b,coordinate_frame):
    ...
    hp.mollview(hpx_map, title=r"Antares all tracks, $\Lambda>5$,$\beta<1$",hold=True)
    hp.graticule()
...
if __name__ == "__main__":
    ...
    #    aaaaa=dfdd.iloc[a]
    #    aaa=list(aaaaa[['off0','off1','off2','off3','off4','off5','off6','off7','off8','off9']])
    #    counter=Counter(aaa)
    #    if counter[1]>1:
    #        lista.append(a)
    #print(len(lista))
    ...
    #print(modDfObj)
    ONandOFF(dfdd)    
    #plotter(dfdd['gal_l'],dfdd['gal_b'],'galactic')
